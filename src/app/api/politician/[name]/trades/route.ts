...
function slugToName(slug: string, nameList: string[]) {
...
export async function GET(req: NextRequest, { params }: { params: { name: string } }) {
  try {
    ...
    if (nameList.length === 0) {
      ...
      nameList = Array.from(new Set(data.map((item: any) => item.Name)));
    }
    const realName = slugToName(slug, nameList) || "Nancy Pelosi";

    // 캐시 확인
    if (cache[slug] && now - cache[slug].time < CACHE_DURATION) {
      const filtered = cache[slug].data.filter((item: any) => item.Name === realName);
    ...
    const data = await res.json();
    // 해당 정치인의 데이터만 필터링
    const filtered = data.filter((item: any) => item.Name === realName);
    // 캐시 저장
